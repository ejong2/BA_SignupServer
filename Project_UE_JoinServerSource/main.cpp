...
unsigned WINAPI WorkThread(void* Args)
{
    ...
    while (true)
    {
        ...
        RecvBytes = recv(CS, Buffer2, sizeof(Buffer2), 0);
        if (RecvBytes <= 0)
        {
            cout << "클라이언트 연결 종료 : " << CS << '\n';
        ...
        Buffer2[PACKET_SIZE - 1] = '\0';
        string strPWD = Buffer2;

        pstmt = con->prepareStatement("INSERT INTO GMUSER(`USER_ID`,`USER_PWD`) VALUES(?, ?)");
        ...
        for (int i = 0; i < vSocketList.size(); i++)
        {
            ...
            do
            {
                SendBytes = send(vSocketList[i], &Buffer2[TotalSentBytes], sizeof(Buffer2) - TotalSentBytes, 0);
                TotalSentBytes += SendBytes;
            } while (TotalSentBytes < sizeof(Buffer2));
